...
public class RelativeCanvas {
    ...
    private final int endPointSpace = ViewUtils.dip2px(2);
    ...
    private void drawLineWithText(Canvas canvas, int startX, int startY, int endX, int endY) {
        ...
        if (startX == endX) {
            ...
            String text = ViewUtils.px2dip(endY - startY) + "dp";
            drawText(canvas,
                    text,
                    startX + textLineDistance,
                    startY + (endY - startY) / 2 + ViewUtils.getTextHeight(textPaint, text) / 2);
        } else if (startY == endY) {
            drawLineWithEndPoint(canvas, startX + endPointSpace, startY, endX - endPointSpace, endY);
            String text = ViewUtils.px2dip(endX - startX) + "dp";
            drawText(canvas,
                    text,
                    startX + (endX - startX) / 2 - ViewUtils.getTextWidth(textPaint, text) / 2,
