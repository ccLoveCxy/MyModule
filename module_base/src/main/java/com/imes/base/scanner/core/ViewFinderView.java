...
//            drawLaser(canvas);
    ...
    private void drawScanLight(Canvas canvas) {
        ...
        if (scanLineTop >= frame.bottom-30) {
            ...
            scanLineTop += 30;// SCAN_VELOCITY可以在属性中设置，默认为5
        }
        Rect scanRect = new Rect(frame.left, scanLineTop, frame.right, scanLineTop + 5);
        canvas.drawRect(scanRect, mLaserPaint);

        postInvalidateDelayed(ANIMATION_DELAY,
                scanRect.left - POINT_SIZE,
                scanRect.top - POINT_SIZE,
                scanRect.right + POINT_SIZE,
                scanRect.bottom + POINT_SIZE);
    }
    @Override
