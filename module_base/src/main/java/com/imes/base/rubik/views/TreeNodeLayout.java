...
public class TreeNodeLayout extends LinearLayout {
    ...
    public void setLayerCount(int layerCount, int sysLayoutCount) {
        ...
        setPadding(interval * layerCount + ViewUtils.dip2px(2), getPaddingTop(), getPaddingRight(), getPaddingBottom());
        invalidate();
    }

    private final int interval = ViewUtils.dip2px(8);
    private Paint paint = new Paint() {
        {
            setColor(Color.GRAY);
            setStyle(Style.FILL);
            setStrokeWidth(ViewUtils.dip2px(0.5f));
        }
    };
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 1; i <= layerCount; i++) {
            if (i > sysLayoutCount) {
                ...
                paint.setStrokeWidth(ViewUtils.dip2px(1));
            } else {
